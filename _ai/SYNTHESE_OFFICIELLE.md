...
| 20 | 🟡 Partiellement résolu | Workflow `Tests & Quality` : hang post-tests **résolu 2026-05-14** via `forceExit: true` dans `jest.config.js` (commit `e704e45`). Le hang masquait 5 familles de bugs (cf. #24-28). Tests qui passent : i18n, i18n.integrity, health, Promise, lint (49 tests verts). | `backend/jest.config.js` + `backend/__tests__/*.test.js` | Hang fixé. Les 5 familles de bugs sous-jacents sont à traiter individuellement en Phase F. |
...
| 24 | 🟠 Élevée | **Vestiges CJS dans projet ESM** : 8 fichiers de tests legacy dans `backend/tests/` (auth.test.js, ideas.test.js, integration/*.test.js, unit/*.test.js) utilisent `require()` alors que le projet est `"type": "module"`. Tous remontent `ReferenceError: require is not defined` lors du run Jest. Détectés post-`forceExit` 2026-05-14. | `backend/tests/**/*.test.js` | Supprimer le dossier `tests/` OU migrer chaque fichier vers ESM (`import`). Doublon avec `__tests__/` qui est la convention actuelle. À traiter Phase F. |
| 25 | 🟠 Élevée | **Tests référençant modules supprimés** : `Gamification.test.js` (`ActionLoggerService.js` absent), `pde.test.js` (`IngestionService.js` absent), `Admin.test.js` (`Badge` non exporté depuis `models/index.js`). Modules retirés lors du nettoyage post-over-engineering mais tests jamais mis à jour. | `backend/__tests__/{Gamification,pde,Admin}.test.js` | Marquer en `describe.skip(...)` ou supprimer ces fichiers. Tous concernent des modules gelés post-MVP (cf. §10). À traiter Phase F. |
| 26 | 🟡 Moyenne | **Validations Elu désynchronisées** : `transparency.test.js` crée des élus avec un champ `niveau` dont les valeurs ne passent plus la validation Sequelize `isIn` du modèle Elu actuel. Cascade de 18 FAIL. | `backend/__tests__/transparency.test.js` + `backend/src/models/Elu.js` | Aligner les valeurs `niveau` du test sur celles du modèle (ou inversement). À traiter Phase F. |
| 27 | 🟠 Élevée | **Désynchronisation table signatures** : `signatures.test.js` et `sign-petition.test.js` font des requêtes SQL brutes via `pool.js` sur la table `petition_signatures`, mais le modèle Sequelize actuel utilise probablement la table `signatures`. Résultat : 500 Internal Server Error sur POST/DELETE /sign. | `backend/src/db/pool.js` + `backend/__tests__/sign*.test.js` + `backend/__tests__/signatures.test.js` | Identifier le bon nom de table (`Signature.tableName`) et harmoniser. À traiter Phase F. |
| 28 | 🟡 Moyenne | **Sequelize drop après close** : `transparency.test.js` afterAll appelle `sequelize.drop()` puis `sequelize.close()` dans le mauvais ordre OU sur une instance déjà fermée par un autre test. Erreur `Cannot set properties of undefined (setting 'supportsSearchPath')`. | `backend/__tests__/transparency.test.js` (lignes ~137-140) | Inverser ordre (drop avant close) OU retirer drop si la BD test est éphémère. À traiter Phase F. |
| 29 | 🟢 Faible | **Workflow Deploy strict HTTP 201** : `deploy.yml` rejette toute réponse autre que HTTP 201 de l'API Render. Render répond HTTP 202 (Accepted) quand un deploy est déjà en file d'attente (cas observé 2026-05-14 lors de 3 push rapprochés). Le workflow échoue alors que le redeploy natif Render via push Git fonctionne normalement en parallèle. | `.github/workflows/deploy.yml` ligne ~75 | Élargir le test à `201 OR 202` : `if [ "$http_code" != "201" ] && [ "$http_code" != "202" ]`. Impact réel nul (auto-redeploy natif Render fonctionne). À traiter Phase F. |
| 30 | ✅ Résolu | **Express trust proxy non configuré** : warning `ERR_ERL_UNEXPECTED_X_FORWARDED_FOR` répété dans les logs Render. Express par défaut `trust proxy = false`, mais Render injecte le header `X-Forwarded-For`. Conséquence : `express-rate-limit` calcule le rate limit sur l'IP du proxy Render, pas sur l'IP client → mitigation IP non opérante. | `backend/src/server.js` | ✅ Résolu 2026-05-22 : `app.set('trust proxy', 1)` ajouté avant Helmet. Rate-limit IP opérant + warning éliminé. |
| 31 | 🟠 Élevée | **Fixtures tests désynchronisées API actuelle** : `__tests__/api.test.js`, `signatures.test.js`, `sign-petition.test.js` attendent l'ancien wrapper `{ success, message, data }` mais le code production retourne désormais le format Phase E `{ signed, totalSignatures }` pour les routes sign + `'Pétition créée avec succès'` au lieu de `'Pétition créée'`. Idem assertions de pagination obsolètes. Découvert lors du cleanup Phase F partiel (2026-05-22). | `backend/__tests__/{api,signatures,sign-petition,comments,petitions-list}.test.js` | Reconstruction Phase D — réécrire fixtures pour le format API actuel. Hors périmètre cleanup partiel. |
| 32 | 🟡 Moyenne | **Test magic-link sans mock SMTP** : `api.test.js` ligne 214 `'should create email verification token'` timeout 5s parce que le test envoie un vrai email via Brevo en environnement test. Découvert 2026-05-22. | `backend/__tests__/api.test.js` | Mocker `EmailService` ou skip si `NODE_ENV=test` sans `BREVO_SMTP_*` configuré. Hors périmètre cleanup partiel. |
| 33 | 🟡 Moyenne | **`promises.test.js` casse l'état BD locale** : `afterAll` appelle `await sequelize.drop()` (drop complet du schéma), forçant re-seed après chaque `npm test`. Imports désynchronisés (`../src/config/database.js` vs `../src/db/sequelize.js`, User en named import). Test file en réalité non chargeable au démarrage Jest mais le `drop` est destructeur s'il était jamais exécuté. Découvert 2026-05-22. | `backend/__tests__/promises.test.js` | Reconstruction Phase D — réécrire le fichier avec les bons imports + retirer le drop + scope-cleanup ciblé. Hors périmètre cleanup partiel. |
...
| 2026-05-14 | Opérateur | **🔒 Session post-migration Neon — Phase 1 Consolidation** : commit `44f3e80` (`docs(synthesis): migration BD Neon réussie + règles 6.3/6.4 DATABASE_URL`) poussé sur `origin/main`. Verrouille la traçabilité des modifs de la session migration (CLAUDE.md §6.3+§6.4, SYNTHESE §3/§14/§16/§22). Auto-redeploy Render OK, /health 200, latence 27 ms post-redeploy. |
| 2026-05-14 | Opérateur | **✅ Vérification stabilité Neon T+5h** : `/health` (status ok, database connected), `/api/v1/petitions` (3 pétitions Québec ids 6/7/8 published), `/api/v1/elus` (6 élus dont Caroline Matte id 6), logs Render (0 erreur Sequelize, 2 démarrages propres), Neon usage (compute 0.02/100 CU-hrs, storage 0.03/0.5 GB, transfer 0/5 GB → marge >97%). Migration tient parfaitement. |
| 2026-05-14 | Opérateur | **📋 Plan de surveillance 7j créé** : `_todo/surveillance_neon.md` (3 checkpoints datés J+1/J+3/J+7 + critères rollback complets vers Render Internal URL si Neon plante avant suppression). Commit `6ef31eb` poussé sur main. |
| 2026-05-14 | Opérateur | **🐛 Bug #20 PARTIELLEMENT RÉSOLU — hang Jest** : ajout `forceExit: true` à `backend/jest.config.js` (commit `e704e45`). Le hang post-tests disparaît immédiatement. 49 tests passent (i18n, i18n.integrity, health, Promise, lint). Mais Jest révèle alors **5 familles de bugs sous-jacents** que le hang masquait : (A) vestiges CJS dans `tests/` (8 fichiers), (B) tests référençant modules supprimés (Gamification, pde, Admin), (C) validations Elu désynchronisées (transparency), (D) erreur SQL table signatures vs petition_signatures (sign-petition, signatures), (E) sequelize.drop après close (transparency afterAll). Consignés en §14 #24-28. À traiter Phase F. |
| 2026-05-14 | Opérateur | **📝 Bugs additionnels consignés** : #29 (Deploy workflow strict HTTP 201, rejette HTTP 202 de Render quand deploy en file — observé sur push rapprochés) + #30 (Express `trust proxy` non configuré → warning `ERR_ERL_UNEXPECTED_X_FORWARDED_FOR`, rate-limit IP non opérant). Tous deux faibles sévérité. |
| 2026-05-22 | Opérateur | **🗺️ Lot 1 — Carte fonctionnelle livrée (Phase G.1)** : refonte `frontend/src/components/Map.jsx` (TileLayer CARTO Dark Matter, centre pilote `[46.8139, -71.2080]` Vieux-Québec zoom 12, cercle pilote rose 7 km, icônes DivIcon SVG distinctes élu rose `#ec4899` / pétition cyan `#06b6d4`, popups typés). Refonte `Map.css` (palette CitoyenAvise, popups, marqueurs). Refonte `frontend/src/styles/map.css` (barre filtres Type + Région + compteur). Refonte `frontend/src/pages/MapPage.jsx` (chargement parallèle élus + pétitions, fusion typée, décalage déterministe 280 m des pétitions autour de l'élu visé). Extension `backend/src/routes/petitions.js` (`GET /api/v1/petitions` retourne désormais `elu.latitude` + `elu.longitude`). Suppression `EluMarker.jsx` + `EluMarker.css` (orphelins, API divergente). **Correction Lot 1.6** : retrait complet de `leaflet.markercluster` (spider-expansion non désirée) → remplacement par `L.layerGroup()` pour affichage individuel des marqueurs. Validation visuelle propriétaire OK. Aucun commit ni push à ce stade. |
| 2026-05-22 | Opérateur | **🏷️ Lot 2 — Référentiel enjeux livré (Phase G.1)** : ajout colonne `enjeu VARCHAR(20)` à la table `petitions` via migration idempotente `V011_petition_enjeu.sql` + script one-off `backend/scripts/migrate-v011-petition-enjeu.js` (dotenv local) + endpoint admin `POST /api/v1/admin/migrate-petition-enjeu` (token-protected, idempotent, backfill inclus). 8 catégories civiques autorisées : `taxes, logement, sante, elections, droits, environnement, energie, autre` + CHECK constraint + index partiel `idx_petitions_enjeu`. Mise à jour modèle `Petition.js` (validateur `isIn`). Extension `backend/src/routes/petitions.js` : Zod `listPetitionsQuerySchema` + `createPetitionSchema` + `updatePetitionSchema` acceptent `enjeu`, filtre WHERE dans GET /, attribut retourné dans GET / + GET /:id + POST / + PUT /:id. Seeders `seed.js` et `admin-seed.js` enrichis : pistes cyclables → `environnement`, RTC → `autre`, Sainte-Foy → `environnement`. Frontend `MapPage.jsx` : 9 pills (Tous + 8 catégories) avec filtre client, état local, désactivation quand Type=Élus. Frontend `Map.jsx` : badge enjeu coloré dans popup pétition (palette par catégorie). Frontend `map.css` + `Map.css` étendus. Migration locale exécutée OK (BD locale, 3 pétitions seed enjeu peuplé). Validation visuelle propriétaire OK. Aucun commit ni push à ce stade. **À exécuter en prod (Neon)** : `curl -X POST -H "Authorization: Bearer $ADMIN_SEED_TOKEN" https://api.citoyenavise.org/api/v1/admin/migrate-petition-enjeu`. |
| 2026-05-22 | Opérateur | **🧹 Cleanup Phase F partiel — bugs #24/#25/#26/#27/#28/#30** : (1) Bug #30 `trust proxy` Express ajouté dans `backend/src/server.js` (`app.set('trust proxy', 1)` avant Helmet) — rate-limit IP désormais opérant + warning `ERR_ERL_UNEXPECTED_X_FORWARDED_FOR` éliminé. (2) Bug #24 vestiges CJS : suppression complète du dossier `backend/tests/` (8 fichiers legacy `require()` dans projet ESM, doublons de `__tests__/`). (3) Bug #25 modules gelés : suppression de `backend/__tests__/Gamification.test.js`, `pde.test.js`, `Admin.test.js` (référençaient `ActionLoggerService`, `IngestionService`, `Badge` modules retirés post-MVP, cf. §10). (4) Bug #26 validation Elu désynchronisée : `transparency.test.js` aligné sur modèle Elu (`niveau='fédéral'` au lieu de `'federal'`, `titre='Conseiller'` au lieu de `'Conseillère'`, retrait champ `prenom` inexistant). Route `transparency.js` enrichie d'un mapping ASCII→accent (`NIVEAU_MAP['federal']='fédéral'`) pour garder l'API agnostique aux accents. (5) Bug #27 désynchronisation table signatures : suppression du fichier vestige `backend/src/__tests__/signatures.test.js` (utilisait `petition_signatures` table inexistante via raw SQL). Le dossier `backend/src/__tests__/` (1 fichier orphelin) est retiré. Les tests modernes dans `backend/__tests__/` utilisent déjà le modèle `Signature` (tableName=`signatures`). (6) Bug #28 transparency afterAll : `sequelize.drop()` + `sequelize.close()` (qui cassait les tests suivants) remplacés par destruction ciblée des entités créées par le test. Famille A complémentaire : `sync({alter:true})` → `sync({alter:false})` dans Promise.test.js + promises.test.js + transparency.test.js (alter cassait sur `Circonscription.elus_ids` ARRAY). `sequelize.close()` retiré de 9 fichiers de tests (cascade `ConnectionManager` closed). **Bugs résiduels documentés en #31/#32/#33** : fixtures API désynchronisées, magic-link sans mock SMTP, `promises.test.js` destructeur. Reconstruction Phase D nécessaire. Aucun commit ni push à ce stade. |
