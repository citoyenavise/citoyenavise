...
| 30 | 🟢 Faible | **Express trust proxy non configuré** : warning `ERR_ERL_UNEXPECTED_X_FORWARDED_FOR` répété dans les logs Render. Express par défaut `trust proxy = false`, mais Render injecte le header `X-Forwarded-For`. Conséquence : `express-rate-limit` calcule le rate limit sur l'IP du proxy Render, pas sur l'IP client → mitigation IP non opérante. Pas bloquant, mais affaiblit la protection anti-abus. | `backend/src/server.js` | Ajouter `app.set('trust proxy', 1);` au démarrage Express. À traiter Phase F. |
