...
async function createTestPetition(creator, elu, status = 'published', signaturesCount = 0) {
...
describe('Pétitions - Suite d\'intégration complète', () => {
  ...
  let elu1, elu2;
  let petition1, petition2, petition3;
  let jwt1, jwt2, jwt3;
  ...
  describe('GET /api/v1/petitions', () => {
    ...
    it('returns pagination metadata', async () => {
      const response = await request(app)
        .get('/api/v1/petitions?page=1&limit=10');
  ...
  describe('DELETE /api/v1/petitions/:id/sign', () => {
    ...
    it('removes signature from petition', async () => {
      const countBefore = (await request(app).get(`/api/v1/petitions/${tempPetition.id}`)).body.data.signaturesCount;
  ...
  describe('Workflow complet', () => {
    ...
    it('2. User adds comment', async () => {
      ...
      expect(response.status).toBe(201);
      expect(response.body.data.contenu).toBe('I support this petition initiative');
    });

    it('3. Stats show updated counts', async () => {
      const response = await request(app)
        .get(`/api/v1/petitions/${workflowPetition.id}/stats`);
    ...
    it('5. Signatures count decreased', async () => {
      const response = await request(app)
        .get(`/api/v1/petitions/${workflowPetition.id}/stats?goal=100`);
